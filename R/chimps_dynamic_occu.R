  ...
  species <- read.csv("../Data/occupancy//chimpSmallGridUnion.csv")
...
#notSurveyd2019 <- c(483, 508, 484, 509, 534, 559, 485, 510, 535, 560, 585, 366, 391, 416, 441, 367, 392, 417, 442, 368, 393, 418)
 ...
 # yearList[[length(yearList)]][which(largeGridVars$largeGridID %in% notSurveyd2019),] <- NA
  ...
  # Like fm5 with mean elevation and mean dist r dependence of 1st-year occupancy
...
write.csv(ms@Full, "../Results/chimpModCoefs.csv")
write.csv(modTabs, "../Results/chimpModAIC.csv")
...
# Distance to road
  nd <- data.frame(meanDistR = seq(min(siteCovs$meanDistR), max(siteCovs$meanDistR), length.out = nrow(siteCovs)), meanElev = 0)
  predDistR <- unmarked::predict(models, type = "psi", newdata = nd, appendData = TRUE) # initial occupancy
  plot(plogis(predDistR$Predicted) ~ predDistR$meanDistR, ylim = c(0.2,1), type = "l")
  lines(plogis(predDistR$upper) ~ predDistR$meanDistR, lty = 2)
  lines(plogis(predDistR$lower) ~ predDistR$meanDistR, lty = 2)
...
projected(fm6)
fm6 <- nonparboot(fm6, B = 500)
tempChange <- data.frame(cbind(projected=projected(fm6)[2,], SE=fm6@projected.mean.bsse[2,]))
...
pdf("../Results/chimpAnnualOccu.pdf", width = 4, height = 4)
...
newDat <- siteCovs
psiPred <- unmarked::predict(fm6, type = "psi", newdata = newDat, appendData = TRUE)
...
pdf("../Results/chimpOccuMap.pdf", width = 4, height = 4)
...
confintList <- list(
confint(fm6, type = "psi"),
confint(fm6, type = "col"),
confint(fm6, type = "ext"),
...
write.csv(modCoefs, "../Results/chimpTopOccuConfints.csv")

save.image("../Results/chimpOccupancy.RData")
